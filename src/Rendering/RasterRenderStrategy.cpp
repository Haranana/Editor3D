...
void RasterRenderStrategy::render(RenderableObject3D& object, Renderer& renderer)
{
    ...
    for (int it = 0; it < static_cast<int>(object.faceVertexIndices.size()); it += 3) {
        ...
        /*
        minX = (int)std::max(0, (int)std::floor(minY));
        maxX = (int)std::min(renderer.getRenderingSurface()->getImg()->height()-1, (int)std::floor(maxY));
        minY = (int)std::max(0, (int)std::floor(minX));
        maxY = (int)std::min(renderer.getRenderingSurface()->getImg()->width()-1, (int)std::floor(maxX));
        */
